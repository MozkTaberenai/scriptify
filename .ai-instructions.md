...
3. **Pre-commit checks** (MANDATORY):
   ...
   # CRITICAL: Check for formatting changes and commit them
   git status  # Check if fmt made any changes
   # If files are modified by formatting:
   git add .
   git commit -m "fix: apply rustfmt formatting changes"
   
   # Verify formatting is clean
   cargo fmt -- --check  # MUST show no errors
   
   # Option 2: Individual commands
   cargo test
   cargo xtask clippy  # Comprehensive clippy checks
   cargo xtask fmt
   git status && git add . && git commit -m "fix: apply formatting" # If changes
...
3. **Before committing** (MANDATORY - NO EXCEPTIONS):
   ...
   # git status && git add . && git commit -m "fix: formatting" # If changes
   # cargo xtask readme    # If src/lib.rs docs were changed
   
   # Step 5: Commit your actual changes
   git add .
   git commit -m "feat: descriptive commit message"
   git push origin feature/branch-name
   
   # Step 6: Create Pull Request (if ready for review)
...
# 4. CRITICAL: Handle formatting changes first
git status  # Check if rustfmt made changes
...
# 6. Add README if docs changed
cargo xtask readme     # If src/lib.rs docs changed

# 7. Commit your actual changes and push
git add .
...
### CRITICAL: Formatting Commit Process
⚠️ **NEVER SKIP**: Formatting changes MUST be committed separately to prevent CI failures

**Step-by-step process:**
1. `cargo xtask precommit` (runs test + clippy + fmt automatically)
2. `git status` (check for changes made by rustfmt)
3. **IF formatting changes exist**: `git add . && git commit -m "fix: apply rustfmt formatting changes"`
4. `cargo fmt -- --check` (verify no formatting issues remain - MUST show no errors)
5. `cargo xtask readme` (if src/lib.rs docs changed)
6. Continue with your actual work commits

**RECOMMENDED: Use all-in-one command with formatting check:**
```bash
cargo xtask precommit
git status  # Check for fmt changes
# If changes: git add . && git commit -m "fix: apply formatting"
cargo fmt -- --check  # Verify clean
cargo xtask readme  # If needed
```
...
4. `git status && git add . && git commit -m "fix: formatting"` (if changes)
5. `cargo fmt -- --check` (verify no formatting issues)
6. `cargo xtask readme` (if src/lib.rs docs changed)
...
- **FORMATTING RULE**: ALWAYS commit formatting changes separately before work commits
- **CI RULE**: `cargo fmt -- --check` MUST pass before pushing (no exceptions)
...
- **Rustfmt failures**: Always run `cargo fmt -- --check` after `cargo xtask precommit`
- **Missing formatting commits**: Check `git status` after formatting and commit changes
- **Mixed commits**: Separate formatting commits from feature commits for cleaner history