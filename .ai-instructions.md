...
4. **MANDATORY**: Run `cargo clippy --all-targets --all-features -- -D warnings` to check for lint issues
5. **MANDATORY**: Run `cargo clippy --tests -- -D warnings` for test-specific lints
6. Understand the pipeline implementation (native vs fallback)
7. Check if README regeneration is needed after changes
