...
fn test_invalid_working_directory() {
    let nonexistent_path = "/this/path/definitely/does/not/exist/12345";

    let result = cmd!("pwd").current_dir(nonexistent_path).no_echo().run();
...
fn test_permission_denied_scenario() {
    // Test writing to a typically restricted directory
    // Skip if running as root
    if env::var("USER").unwrap_or_default() != "root" && env::var("EUID").unwrap_or_default() != "0"
    {
        let result = cmd!("touch", "/etc/test_file_should_fail").no_echo().run();
        assert!(
            result.is_err(),
            "Should fail when trying to write to /etc as non-root"
        );
    }
}
...
fn test_environment_inheritance() {
    ...
    unsafe {
        env::set_var("SCRIPTIFY_TEST_VAR", "original_value");
    }

    // Test that system env vars are inherited
    let output = cmd!("printenv", "SCRIPTIFY_TEST_VAR")
        .no_echo()
        .output()
        .unwrap();
    ...
    let output = cmd!("printenv", "SCRIPTIFY_TEST_VAR")
        .env("SCRIPTIFY_TEST_VAR", "overridden_value")
        .no_echo()
        .output()
        .unwrap();
    ...
    unsafe {
        env::remove_var("SCRIPTIFY_TEST_VAR");
    }
}
